  ...
  <body>
    ...
    <div class="tab-container">
      <div class="tab-buttons">
        ...
        <button onclick="openTab('tab3')">Graphing</button>
      </div>
	  ...
	  <input type="text" id="send_command_input" placeholder="MESC command">
	  <br>
	  <br>

	  <button onclick="logTextBox()">Name</button>
	  <input type="text" id="datetimeInput" placeholder="log name here">
	  <a href="#" onclick="fillDateTime()" style="font-size: 8px;">(date/time)</a>

	  <br>
	  <br>
	  <button id="logButton" onclick="logStartStop()">LOG</button>
	...
	<button onclick="graphCommand()">Graph</button>
	<br>
	<br>
	<div id="container3">
    ...
    <script>
	  ...
	  // Set interval to send date every 10 seconds
	  setInterval(sendDateTime, 10000);
      }
      ...
      function logTextBox() {
	  var inputBox = document.getElementById("datetimeInput");
	  var textBoxValue = inputBox.value;
	  if (textBoxValue) {
	      var cmd = "LOG_NAME: " + textBoxValue;
              websocket.send(cmd);
              console.log('Set log name:', textBoxValue);
	      inputBox.value = '';
              inputBox.placeholder = 'log file name';	      
	  }
	  else {
	      console.log("No text box content");
	  }
      }
	  ...
	  hours = hours % 12 || 12;
	  const formattedDate = `${month}_${now.getDate()}_${hours}:${minutes}${period}`;
	  ...
	  const period = hours >= 12 ? "pm" : "am";
	  hours = hours % 12 || 12;
	  const formattedDate = `TIME_STAMP: ${month}_${now.getDate()}_${hours}:${minutes}${period}`;

	  document.getElementById("datetimeInput").value = formattedDate;

	  // Send WebSocket message if the connection is open
	  if (websocket.readyState === WebSocket.OPEN) {
              websocket.send(formattedDate);
      ...
      // handles websocket messages coming from webserver
      //  content is typically json. Look for calls like:
      //   g_webSocket->textAll(line);
      // that send content to this function
      function onMessage(event) {
	  if (!event.data) {
	      console.warn('Received null from WebSocket ¯\_(ツ)_/¯');
	      return;
	  }
	  try {
	      // Check if event.data is not null or empty
	      if (event.data) {
		  ...
		  // Optionally, append a message to the #messages div
		  const messagesDiv = document.getElementById('messages');
		  const messageElement = document.createElement('div');
		  messageElement.className = 'message';
		  messageElement.textContent = 'Received from MESC: ' + event.data;
		  messagesDiv.appendChild(messageElement);
	      }
	  } catch (error) {
	      console.error('This expects JSON: ', error);
      ...
      function sendCommand() {
          var inputBox = document.getElementById('send_command_input');
          var command = inputBox.value;
          if (command) {
	      var cmd = "MESC: " + command;
              websocket.send(cmd);
	  ...
	  if (!isLogging) {
              console.log('Send log_start:');
              websocket.send("LOG_START:");

              logButton.classList.add("logging-active");
	  } else {
              console.log('Send log_stop:');
              websocket.send("LOG_STOP:");

              logButton.classList.remove("logging-active");
	  }
	  isLogging = !isLogging;
      }
      ...
      async function updateButtonPress() {
          ...
          try {
              const response = await fetch('/updateButton');
...
function createGraph(data) {
    ...
    var desc = data['time'] || ["00:00", "00:15", "00:30"]; // Example time labels
    desc = desc.map(function(value) {
        return value * 10000; // Adjust scaling as needed
    });
