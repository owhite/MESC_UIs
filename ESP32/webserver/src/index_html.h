...
<html lang="en">
  ...
  <body>
    ...
    <div class="tab-container">
	  ...
	  <br>
	  <br>
	  <button onclick="logSetName()">Name</button>
	  <input type="text" id="log_set_name" placeholder="log file name">
	  <br>
	  <button onclick="logStart()">LOG_SRT</button>
	  <br>
	  <button onclick="logStop()">LOG_STP</button>
        </div>
      </div>
    ...
    <script>
	  ...
	  try {
		  ...
		  updateTable(data);
              ...
              // Append a message to the #messages div indicating JSON was received
              messageElement.textContent = 'Received JSON: ' + event.data;
	  } catch (error) {
              // Handle non-JSON strings
	      //   it's not really an error
              messageElement.textContent = 'Received from nonJSON ' + event.data;
	  }

	  // Append the message element to the #messages div in both cases
      ...
      function onLoad(event) {
          ...
          document.getElementById('log_set_name').addEventListener('keydown', function(event) {
              if (event.key === 'Enter') {
                  logSetName();
              }
          });
      ...
      function logSetName() {
          var inputBox = document.getElementById('log_set_name');
          var command = inputBox.value;
          if (command) {
	      var cmd = "LOG_NAME: " + command;
              websocket.send(cmd);
              console.log('Set log name:', cmd);
              inputBox.value = '';
      ...
      function logAddLine() {
          console.log('Send log_add_line:');
          websocket.send("log_add_line");
      }
